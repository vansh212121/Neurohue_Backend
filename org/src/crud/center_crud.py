...
class CenterRepository(BaseRepository[Center]):
    """Repository for all database operations related to the Center model."""
    ...
    async def get_by_center_code(
        self, *, db: AsyncSession, center_code: str
    ) -> Optional[Center]:
        statement = select(self.model).where(self.model.center_code == center_code)
        result = await db.execute(statement)
        return result.scalar_one_or_none()
    ...
    async def get_by_email(self, *, db: AsyncSession, email: str) -> Optional[Center]:
    ...
    async def create(self, db: AsyncSession, *, db_obj: Center) -> Center:
        """Create a new center. Expects a pre-constructed Center model object."""
        db.add(db_obj)
        await db.commit()
    ...
    def _apply_filters(self, query, filters: Dict[str, Any]):
        ...
        if "status" in filters and filters["status"]:
            conditions.append(Center.status == filters["status"])

        if "region_id" in filters and filters["region_id"]:
            conditions.append(Center.region_id == filters["region_id"])
        ...
        if "search" in filters and filters["search"]:
            ...
            conditions.append(
                or_(
                    ...
                    Center.center_code.ilike(search_term),
                    Center.state.ilike(search_term),
                    Center.city.ilike(search_term),
                    Center.email.ilike(search_term),
                    Center.zip_code.ilike(search_term),
                    Center.phone_number.ilike(search_term),
                )
            )
